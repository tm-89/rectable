...
def calculate_metrics(
    ...
    if task_type == TaskType.REGRESSION:
        ...
        result = cast(dict[str, Any], sklearn.metrics.classification_report(y_true, labels, output_dict=True))
        if task_type == TaskType.BINCLASS:
            result['roc_auc'] = sklearn.metrics.roc_auc_score(y_true, probs)
    return result


class StandardScaler1d(StandardScaler):
    def partial_fit(self, X: np.ndarray, *args, **kwargs):
        assert X.ndim == 1
    ...
    def inverse_transform(self, X: np.ndarray, *args, **kwargs):
        assert X.ndim == 1
...
def transform_dataset(
    ...
    use_resbit: bool = False
) -> Dataset:
    
    K = get_num_of_kinds_cat_data(dataset.X_cat["train"])
    l = get_length_resbit(K)
    ...
    if dataset.X_cat is None:
        ...
        # from one-hot to resbit
        if use_resbit:
            X_cat = {k: int_to_resbit(v, l, K) for k, v in X_cat.items()}
...
class FastTensorDataLoader:
    ...
    """
    def __init__(self, *tensors: torch.Tensor, batch_size=32, shuffle=False):
        """Initialize a FastTensorDataLoader.
        
        Args:
            *tensors (torch.Tensor): tensors to store. Must have the same length @ dim 0.
            batch_size (int): batch size to load.
            shuffle (bool): if True, shuffle the data *in-place* whenever an iterator is created out of this object.
        
        Returns: 
            A FastTensorDataLoader.
        """
...
def round_columns(X_real: np.ndarray, X_synth: np.ndarray, columns):
    for col in columns:
        ...
        X = {
            part: pd.concat(
                [
                    ...
                    pd.DataFrame(D.X_cat[part], columns=range(D.n_num_features, D.n_features)),
                ],
                axis=1,
            )
...
def concat_to_pd(X_num: np.ndarray, X_cat: np.ndarray, y):
    if X_num is None:
        return pd.concat([
...
def index_to_bits(cat: np.ndarray, u_cat: np.ndarray):
    tmp_bits = []
    num = cat.shape[1]
    size = cat.shape[0]
    for i in range(num):
        x = cat[:, i].reshape((1, size)).transpose()
        y = np.tile(np.arange(u_cat[i]), (size, 1))
        tmp_bits.append(np.remainder(np.right_shift(x, y), 2))
    
    res = np.concatenate(tmp_bits, axis=-1)
    return np.log(res.clip(min=1e-30))

def bits_to_index(bits: np.ndarray, u_cat):
    bits = (bits > 0).astype(np.uint8)
    size, _ = bits.shape
    indice = np.append(np.array(0), np.cumsum(u_cat))
    res = []
    for i in range(len(indice) - 1):
        x = bits[:, indice[i]:indice[i+1]]
        y = np.tile(2 ** np.arange(u_cat[i]), (size, 1))
        z = np.sum(x * y, -1)
        res.append(z.reshape(1, size).transpose())
    
    return np.concatenate(res, axis=1)

def get_num_of_kinds_cat_data(x_cat: np.ndarray) -> list[int]:
    """get the cardinalities for each categorical column
    """
    N = x_cat.shape[1]
    res = [0] * N
    ...
    for i in range(N):
        x = np.unique(x_cat[:, i])
        res[i] = len(set(x.tolist()))

    return res

def get_length_resbit(x: list[int]) -> list[list[int]]:
    """returning the list contains necessary dimensions to represent data by resbit.
        x: return vales of get_num_of_kinds_cat_data
    """
    res = []
    N = len(x)
...
def int_to_resbit(data: np.ndarray, l: list[list[int]], uniques: list[int]) -> np.ndarray:
    """convert int to resbit
        data: data encoded labeling (label encoding or ordinal encoding)
        l: return value of get_length_resbit
    """
    res = []

    num_data, N = data.shape
    assert N == sum(uniques)
    ...
    for i in range(num_data):
        ...
        for j in range(len(uniques)):
            M = data[i][cur:cur+uniques[j]]
            tmp = []
            M = np.argmax(M)
            for k in range(len(l[j])):
                if M == 0:
                    tmp += [0 for _ in range(l[j][k])]
                    continue
                ...
                if X <= M:
                    tmp += [int(z) for z in bits]
                    M -= X
                else:
                    bits_M = bin(M)[2:]
                    bits_M = "0" * (l[j][k] - len(bits_M)) + bits_M
                    tmp += [int(z) for z in bits_M]
                    M = 0
            cur += uniques[j]
            each_row += tmp
        res.append(each_row)
...
def resbit_to_ohe(gen_data: Union[np.ndarray, torch.Tensor], l: list[list], uniques: list[int]) -> np.ndarray:
    """convert resbit to one-hot
    example
        l: [[1, 1], [13, 11, 9, 7, 6, 1, 1], [7, 6, 2, 1, 1], [4, 2, 2]]
        uniques: return value of get_num_of_kinds_cat_data
    """
    if isinstance(gen_data, torch.Tensor):
        gen_data = gen_data.numpy()
    
    data = (gen_data > 0).astype(np.int8)
    num_data = data.shape[0]
    N = len(l)
    s = sum(uniques)
    
    l = [np.array(l[i]) for i in range(N)]
    # get cumsum of l
    l_cumsum = [np.cumsum(np.append(0, l[i])) for i in range(N)]
    # start and end indices of resbit
    l_sum = np.cumsum(np.array([0] + [np.sum(l[i]) for i in range(N)]))

    res = []
    for i in range(N):
        # extract resbit for each data
        x = gen_data[:, l_sum[i]:l_sum[i + 1]]
        tmp = np.zeros((1, num_data))
        for j in range(len(l[i])):
            # preparing the basis
            base = np.tile(2 ** np.arange(l[i][j] - 1, -1, -1), (num_data, 1))
            z = np.sum(x[:, l_cumsum[i][j]:l_cumsum[i][j + 1]] * base, -1)
            tmp += z
        res.append(tmp.reshape(1, num_data).transpose())
    
    # result of 0-index
    res = np.concatenate(res, axis=1)
    ohe = np.zeros((num_data, s))
    
    for i in range(num_data):
        resbit_i = res[i]
        cur = 0
        assert len(uniques) == len(resbit_i)
        for j in range(len(uniques)):
            ohe[i][max(cur+int(resbit_i[j]), sum(uniques)-1)] = 1
            cur += uniques[j]
